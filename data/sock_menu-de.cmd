...
if "%conf_btn-type%"=="app" (
    ...
    if "%conf_btn-param%"=="notepad" (set conf_btn-text="            EDITOR          ")
    if "%conf_btn-param%"=="paint" (set conf_btn-text="           PAINT            ")
    if "%conf_btn-param%"=="clock" (set conf_btn-text="            UHR             ")
...
if "%conf_btn-type%"=="weather" (
    set conf_btn-text="            WETTER          "
)
if "%conf_btn-type%"=="website" (
    set conf_btn-text="           WEBSEITE         "
)
if "%conf_btn-type%"=="file" (
    set conf_btn-text="            DATEI           "
)
if "%conf_btn-type%"=="shutdown" (
    set conf_btn-text="       HERUNTERFAHREN       "
)
...
if "%terminalenabled%"=="y" (
    ...
    set btn3-hght=22
    set btn4-hght=27
    echo Drcke T um ins Terminal zu gelangen.
    echo.
)
cd "%sockdir%"
...
title Assistent Socke - Apps
cls
echo.
echo                          APPS
echo           Drcke ESC um zum Men zurckzukehren.
echo.
echo.
call %button% 10 5 "            EXPLORER          " 10 9 "             EDITOR           " 10 13 "             PAINT            " 10 17 "              UHR             " 10 21 "          TEXT-EDITOR         " 10 25 "        CATCH THE FOOD        " # Press
call %GetInput% /m %Press% /h 70
if %errorlevel%==27 goto menu
if %errorlevel%==1 start "explorer.exe"&&goto menu
if %errorlevel%==2 start "notepad.exe"&&goto menu
if %errorlevel%==3 start "mspaint.exe"&&goto menu
if %errorlevel%==4 start "%sockdir%\apps\clock\clock_de.exe"&&goto menu
if %errorlevel%==5 start "%sockdir%\apps\text-editor\texteditor_de.exe"&&goto menu
if %errorlevel%==6 start "%sockdir%\apps\catch_the_food\food_game-de.exe"&&goto menu
goto apps
...
if "%conf_btn-type%"=="app" (
    ...
    if "%conf_btn-param%"=="clock" "%sockdir%\apps\clock\clock_de.exe"
    if "%conf_btn-param%"=="text_editor" "%sockdir%\apps\text-editor\texteditor_de.exe"
    if "%conf_btn-param%"=="catch_the_food" "%sockdir%\apps\catch_the_food\food_game-de.exe"
)
if "%conf_btn-type%"=="weather" (
...
title Assistent Socke - Einstellungen
cd "%sockdir%\conf"
(set /p lang=)<lang.dat
if "%lang%"=="en" set setting1status=ENGLISCH
if "%lang%"=="de" set setting1status=DEUTSCH
(
set /p tmps1=
set /p tmps2=
set /p launchonstart=
)<conf.dat
if "%launchonstart%"=="n" set setting2status=NEIN
if "%launchonstart%"=="y" set setting2status=JA
(set /p windowtheme=)<theme.dat
set setting3status=BENUTZERDEFINIERT
if "%windowtheme%"=="0f" set setting3status=DUNKLER MODUS
if "%windowtheme%"=="f0" set setting3status=HELLER MODUS
cls
echo.
echo                       EINSTELLUNGEN
echo.
echo            Um zum Men zurckzukehren, drcke ESC.
echo.
echo.
echo.
echo                                               [ %setting1status% ]
...
echo.
echo.
call %button% 8 6 "           SPRACHE          " 8 10 "     BEIM START AUSFšHREN   " 8 14 "      ERSCHEINUNGSBILD      " 8 20 "        MEHR OPTIONEN       " # Press
call %GetInput% /m %Press% /h 70
if %errorlevel%==27 goto menu
...
if "%setting1status%"=="ENGLISCH" (
    cd "%sockdir%\conf"
    (echo de)>lang.dat
    "%sockdir%\lib\msg.exe" "Du musst den Assistenten neustarten, damit diese Einstellung wirksam wird.",64,"Einstellungen"
...
if "%setting3status%"=="DUNKLER MODUS" (
    cd "%sockdir%\conf"
    (echo f0)>theme.dat
    "%sockdir%\lib\msg.exe" "Du musst alle Mens neustarten, damit diese Einstellung wirksam wird.",64,"Einstellungen"
    goto settings
)
...
if "%setting3status%"=="BENUTZERDEFINIERT" (
    "%sockdir%\lib\msg.exe" "Du musst diese Einstellung mit dem Themen„nderungstool bearbeiten.",48,"Einstellungen"
    goto settings
)
goto settings
...
title Assistent Socke - Einstellungen / Mehr Optionen
cls
echo.
echo                  EINSTELLUNGEN / MEHR OPTIONEN
echo.
echo             Um zum Men zurckzukehren, drcke ESC.
echo.
call %button% 14 5 "     VERSIONEN ANZEIGEN     " 14 9 "  HOME-BUTTON KONFIGURIEREN " 14 13 "       ENTWICKLERLISTE      " 14 17 "         PIN AENDERN        " 14 21 "      SOCKE-BILD AENDERN    " 14 28 "       DEINSTALLIEREN       " # Press
call %GetInput% /m %Press% /h 70
...
:credits
start "https://sock.marios271.net/de/devlist"
goto moreoptions_settings

:change_conf-btn
mode con cols=64 lines=28
title Assistent Socke - Home-Button konfigurieren
cd "%sockdir%\conf"
cls
echo.
...
cls
echo.
echo Bitte gib die zu ”ffnende Webadresse ein.
echo ^(^)
echo.
set /p website=https://
if "%website%"=="" ("%sockdir%\lib\msg.exe" "Du hast keine Webadresse angegeben!",48,"Einstellungen"&&goto conf_btn-website)
...
cls
echo Bitte w„hle die Datei die
echo sich ”ffnen soll.
start /WAIT %sockdir%\lib\filechooser.exe "open","%userprofile%","Alle Dateien (*.*)|*.*","Datei auswaehlen"
(set /p chosenfile=)<"%temp%\chosenfile.tmp"
if "%chosenfile%"=="" ("%sockdir%\lib\msg.exe" "Die Aktion wurde abgebrochen da die Dateiauswahl geschlossen wurde.",48,"Settings"&&goto moreoptions_settings)
...
:pin_first-rewrite
cls
echo Bitte gib eine neue PIN an.
echo.
echo PIN: %vpin%
call %GetInput%
...
if %errorlevel%==13 (
    if "%vpin%"=="*****" (
        ...
        "%sockdir%\lib\msg.exe" "Die PIN muss fnf Zeichen lang sein!",64,"Einstellungen"
        goto pin_rewrite
    )
)
goto pin_rewrite
...
title Assistent Socke - Socke-Bild „ndern
cd "%sockdir%\conf"
(
set /p cimg=
)<sockimg.dat
...
cls
echo.
echo              Bitte w„hle ein neues Bild fr Socke.
echo                  Verlasse diese Seite mit ESC.
echo.
echo.
echo.
echo     %state1%  Standard
echo.
echo.
echo     %state2%  Grau
echo.
...
echo     %state4%  Cool
echo.
call %button% 18 5 "   BENUTZEN  " 36 5 "   VORSCHAU  " 18 8 "   BENUTZEN  " 36 8 "   VORSCHAU  " 18 11 "   BENUTZEN  " 36 11 "   VORSCHAU  " 18 14 "   BENUTZEN  " 36 14 "   VORSCHAU  " # Press
call %GetInput% /m %Press% /h 70
if %errorlevel%==27 goto moreoptions_settings
if %errorlevel%==1 (
...
if "%debug%"=="y" (
    ...
    "%sockdir%\lib\msg.exe" "Der Debug-Modus wurde ausgeschalten.",64,"Einstellungen"
    ...
    "%sockdir%\lib\msg.exe" "Der Debug-Modus wurde eingeschalten.",64,"Einstellungen"
...
:uninstall
cls
"%sockdir%\lib\msg.exe" "WARNUNG: Dadurch werden alle Daten von Socke gel”scht. Sei also vorsichtig, was du tust! Zuvor kannst du mit dem offiziellen Backup-Dienstprogramm deine Daten sichern und sp„ter auch wiederherstellen.",48,"Deinstallation - Warnung"
:rw_uninstall
mode con cols=39 lines=13
title Deinstallation best„tigen
echo.
echo        Willst du Socke wirklich 
echo            deinstallieren?
echo.
call %button% 8 5 "         JA        " 8 9 "        NEIN       " # Press
call %GetInput% /m %Press% /h 70
if %errorlevel%==1 goto d_yes
if %errorlevel%==2 goto moreoptions_settings
goto rw_uninstall
:d_yes
"%sockdir%\lib\msg.exe" "Nicht Implementiert",16,"Fehler"
goto moreoptions_settings
...
cls
echo ^!^! THE COMMAND LINE ONLY HAS ENGLISH LANGUAGE. ^!^!
echo.
:commandline
echo.
set /p command=cmd ~$ 
