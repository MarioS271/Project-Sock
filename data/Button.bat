...
:SLoop
    if "%1"=="" (exit)
    if "%1"=="#" (%userprofile%\Assistants\Sock\lib\button\batbox !Buttons! & endlocal & set "%~2=%Button_Dim%" & exit /b)
    set XBPB=%1
    set YBPB=%2
    set "Text_Button=%~3"
...
rem Some code in line 32 was changed by me with permissions from Psi505
rem Sub to him on YT :)